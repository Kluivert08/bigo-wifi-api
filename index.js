...
// --- ROUTE DE SYNCHRONISATION MIKROTIK (Vitesse 2M sans slash) ---
app.get('/sync_mikrotik', async (req, res) => {
    try {
        console.log("--- Tentative de synchro MikroTik ---");
        
        // On récupère les tickets 'paid' (insensible à la casse : Paid/PAID/paid)
        ...
        // TICKET DE SÉCURITÉ : Toujours présent pour tester la liaison
        output += "TEST01,2M,3600s|"; 

        if (data && data.length > 0) {
            data.forEach(t => {
                const code = t.ticket_code || "NOCODE";
                const speed = t.speed_limit || "2M"; // On force 2M si vide
        ...
        console.log("Envoi au MikroTik:", output);
        res.send(output);
...
app.listen(PORT, () => console.log(`🚀 API BIGO v4 active sur port ${PORT}`));
